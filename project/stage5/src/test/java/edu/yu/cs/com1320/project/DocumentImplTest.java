...
class DocumentImplTest {
  ...
  public void docTest() throws URISyntaxException {
    ...
    @Test
    void TestOnStringTextDocument() throws URISyntaxException {
        URI uri = new URI ("Hello");
        DocumentImpl text = new DocumentImpl(uri, "HE^^l&%lo W^&%^orLD");
        assertEquals("HE^^l&%lo W^&%^orLD", text.getDocumentTxt());
        assertEquals(null, text.getDocumentBinaryData());
        assertEquals(uri, text.getKey());
        assertEquals(1, text.wordCount("hello"));
        assertEquals(1, text.wordCount("world"));
        assertEquals(2, text.getWords().size());
        assertEquals(true, text.equals(new DocumentImpl(uri, "HE^^l&%lo W^&%^orLD")));
    }
    @Test
    void TestOnBinaryTextDocument() throws URISyntaxException {
        URI uri = new URI ("Hello");
        byte[] arr = {1, 2, 3};
        DocumentImpl text = new DocumentImpl(uri, arr);
        assertEquals(arr, text.getDocumentBinaryData());
        assertNull(text.getDocumentTxt());
        assertEquals(uri, text.getKey());
        assertEquals(0, text.getWords().size());
        assertEquals(0, text.wordCount("Hello"));
