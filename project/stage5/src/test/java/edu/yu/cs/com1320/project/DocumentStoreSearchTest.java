...
public class DocumentStoreSearchTest {
        ...
        public void init() throws Exception {
            ...
            doc1 = new DocumentImpl(this.uri1, this.txt1);
            doc2 = new DocumentImpl(this.uri2, this.txt2);
            doc3 = new DocumentImpl(this.uri3, this.txt3);
        }
        ...
        public void basicSearchAndOrganizationTest() throws IOException {
            ...
            assertTrue(store.search("PiE").get(0).getLastUseTime()>doc3.getLastUseTime());
            assertEquals(3, store.searchByPrefix("p").size());
            List<Document> docs = store.searchByPrefix("p");
            long num = docs.get(0).getLastUseTime();
            for (Document d : docs) {
                assertEquals(d.getLastUseTime(), num);
            }
            assertEquals(0, store.searchByPrefix("x").size());
            assertEquals(3, store.searchByPrefix("pi").size());
            assertEquals(doc2, store.search("Pizza").get(0));
            assertEquals(5, store.search("PiZzA").get(0).wordCount("pizza"));
            assertEquals(doc3, store.searchByPrefix("p").get(0));
        }
        @Test
        ...
        public void basicPutOverwriteTest() throws IOException {
            ...
            store.putDocument(new ByteArrayInputStream(this.txt2.getBytes()),this.uri2, DocumentStore.DocumentFormat.TXT);
            assertEquals(2, store.search("pizza").size());
            assertEquals(this.doc2, store.search("pizza").get(0));
            assertEquals(this.doc1, store.search("pizza").get(1));
            assertEquals(this.doc2.hashCode(), store.putDocument(new ByteArrayInputStream(this.txt3.getBytes()),this.uri2, DocumentStore.DocumentFormat.TXT));
            assertEquals(1, store.search("pizza").size());
            assertEquals(this.doc1, store.search("pizza").get(0));
        }
        @Test
        ...
        public void testUndoNoArgs() throws IOException {
            ...
            store.undo();
            assertEquals(2, store.search("pizza").size());
            List<Document> docs = store.search("pizza");
            assertEquals(docs.get(0).getLastUseTime(), docs.get(1).getLastUseTime());
        ...
        public void testUndoCommandSet() throws IOException {
            ...
            assertEquals(doc1, store.search("pizza").get(0));
            assertTrue(!store.search("pizza").contains(doc2));
            store.undo(uri2);
            assertEquals(2, store.search("pizza").size());
            assertEquals(1, store.search("pizza").get(0).getWords().size());
            assertEquals(doc2, store.search("pizza").get(0));
            assertEquals(doc1, store.search("pizza").get(1));
            store.undo();
